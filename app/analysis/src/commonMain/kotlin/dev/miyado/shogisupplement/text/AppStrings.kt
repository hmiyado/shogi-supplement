...
object AppStrings {
    ...
    const val DRILL_QUESTION_PROMPT = "次の1手は？"
    const val DRILL_QUESTION_HINT = "その後の展開も、読んだところまで盤に並べてください。"
    const val DRILL_YOUR_LINE_LABEL = "あなたの読み筋"
    const val DRILL_ANALYSIS_LINE_LABEL = "解析の読み"
    const val DRILL_CLOSE_CONTEST_NOTE = "この局面は有力な手が複数あります。"
    const val DRILL_UNDO_MOVE = "1手戻す"
    const val DRILL_RESET_MOVES = "最初から"
    const val DRILL_SUBMIT_ANSWER = "答える"
    ...
    const val SETTINGS_ROW_FEEDBACK = "X（更新情報・お問い合わせ）"
