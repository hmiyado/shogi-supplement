...
フローは `clearState` で始まるものが多く、そのたびにアプリが匿名アカウントを作る。相手は
本番のSupabaseなので、走らせるだけで利用状況の集計が膨らむ。

Debugビルドは匿名サインアップのメタデータに `platform` を刻み、末尾が `-dev` になる
（`resolvePolicyPlatform`）。集計から除外するのも、あとで消すのも、この印を条件にする。
実行した時間帯で切ると同じ時間帯に入った実ユーザーを巻き込むため、時刻は条件に使わない。

削除は運用側のツールで行う（このリポジトリには入れていない。DBの資格情報を要するため）。
...
`common/` に置いたサブフローはiOS・Android双方の入口から呼ぶ。片方でしか通していない
フローは、もう片方では通らないことがある（1.10で対局者名編集がAndroidで必ず落ちる状態に
なっていた）。追加したら両方で流す。

| 入口 | 見るもの |
| --- | --- |
| 01 | ホームのスモーク |
| 02（Android）／03（iOS） | KIF取込→解析→レポート→対局者名編集→推定棋力詳細→盤入力→学習の記録詳細 |
| 02（iOS） | クリップボードが空のときのエラー |
| 03（Android） | KIFでないファイルのエラー |
| 04 | 手入力 |
| 05 | レポートからの棋譜削除 |
| 06 | 先後選択のキャンセルで下書きが残る |
| 07 | 設定のテーマ変更が他の画面へ反映される |
| 08 | 棋譜一覧の絞り込みと選択して複数削除 |
