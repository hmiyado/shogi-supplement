...
- **アカウントは任意**: サーバー管理したいユーザーだけアカウント作成＋棋譜アップロード。
  ...
  （利用規約に研究利用を明記して同意を取る）
  - **例外（iOSサーバー解析版・2026-07-26決定）**: iOS版は**同意しないと使えない**。
    初回起動で規約＋プライバシーポリシー＋研究利用（データ提供）への同意を必須とし、
    同意＝匿名サインイン。解析完了時にuploaded_gamesへ自動アップロード
    （無料提供の対価が較正・研究データであることを明示）。
    同意の正が `consent_accepted_at` である点はプラットフォーム共通で不変。
    詳細は tmp/server-analysis-design.md「認証・同意モデルの変更」（ローカル計画書）
