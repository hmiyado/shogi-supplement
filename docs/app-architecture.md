...
- **棋譜に残す申告棋力**: 取込時に `game.rating_service` / `rating_raw` / `rating_rule` へ
  写す（サーバーの `uploaded_games` にも同名で送る）。レーティング制は単一値をそのまま、
  段級位制（将棋ウォーズ・棋桜）は棋譜の持ち時間から決めたルールの段級位を
  `ShogiRank.toRaw` の符号化で `rating_raw` に入れ、そのルールIDを `rating_rule` に入れる
  （`declaredRankForGame`）。棋力を申告していなければ3列とも空にする
