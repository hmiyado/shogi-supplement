...
- **What this is:** 自分の実戦棋譜を端末内で解析し、レート帯相応の悪手だけを
  「自分専用の次の一手問題」として周回する将棋学習Androidアプリ（KMP/Compose）
...
| 役割 | 書体 | 用途 |
|---|---|---|
| Display/見出し | **Shippori Mincho**（600/700） | 画面タイトル・次の一手問題の問い・レポート見出し。棋書の風格。**見出し専用**（小サイズ本文には使わない） |
...
| **loss（朱）** | `#C73E3A` | **損失とエラー**: 悪手・勝率損失・不正解・削除・エラー表示（Semantic参照）。装飾には使わない |
...
- **画面モック（ホーム/レポート/次の一手問題の詳細レイアウト）は未確定**。
