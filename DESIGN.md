...
- **枠線・座布団のあるボタン形にしない。** 操作ボタンに見え、いま何を見ているかの表示に読めない
- **選択中は紺青**（文字＋2dpのインジケータ）。非選択は ink2＋細い罫線
- **インジケータは内容のある側の辺に置く**（本文がタブの下なら下辺）
- edge-to-edgeの画面では底に置かない。safe areaぶんの帯が残り、タブと画面の縁が離れる
- 入れ子になる切替（レポートの「本譜／最善の変化」）とは形を変え、縦に並べない

### Adaptive Layout（幅への応答）

- **Breakpoints（ウィンドウ幅・dp）:** compact `<600` ／ medium `600–839` ／
  expanded `≥840`。Material 3のwindow size classに合わせる
- **本文の最大幅:** 600dp。それを超える幅では中央へ寄せ、余った左右はbgのままにする
  （面を敷かない）。**幅を止めるのは本文だけ**——面を持つトップバー・罫線・画面背景は
  端まで通す。面を持たない見出し行は本文と同じ幅へ揃える（端に置くと本文と切り離れる）
- **2ペイン（expandedのみ）:** 盤を持つ画面（レポート・ドリル）は
  左=盤とナビ／右=データ（形勢グラフ・成績・悪手一覧・検討）に1:1で割り、
  境目は縦罫（line）。ペインの中身にも600dpの上限は効く。medium以下は1カラムのまま
- **盤:** マスの上限44dpは幅に依らず据え置く。広い画面でも駒は大きくしない
- No-jitter原則はブレークポイントを跨ぐ変化には適用しない（レイアウトが変わる境界のため）
...
- **解析中の進捗は0.5秒ごとに1手ずつ**進める。届いた順にそのまま出すと、数手飛んでから
  止まる見え方になる。**全局面が揃ったあとは2秒を目安に残りへ追いつく**
  （1手あたり40–100msに収める）。まとめて出すと溜まっていた分が盤に1手も映らないまま
  最終局面へ飛び、間隔を決め打つと長い棋譜ほどレポートの表示が遅れる
